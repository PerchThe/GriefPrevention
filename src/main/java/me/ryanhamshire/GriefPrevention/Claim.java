...
public class Claim
{
    ...
    Claim(Location lesserBoundaryCorner, Location greaterBoundaryCorner, UUID ownerID, List<String> builderIDs, List<String> containerIDs, List<String> accessorIDs, List<String> managerIDs, boolean inheritNothing, Long id, boolean is3D)
    {
        ...
        if (!this.is3D) {
            ...
            // make sure greaterBoundaryCorner has a safe Y too
            this.greaterBoundaryCorner.setY(Math.max(this.lesserBoundaryCorner.getBlockY(), this.greaterBoundaryCorner.getBlockY()));
        }
    ...
    // Height should return Y extent (useful for 3D claims)
    public int getHeight()
    {
        return this.greaterBoundaryCorner.getBlockY() - this.lesserBoundaryCorner.getBlockY() + 1;
    }
            ...
            @Nullable Event event)
    {
        ...
        if (uuid.equals(this.getOwnerID())
                || (playerData != null && playerData.ignoreClaims
        ...
        if (permission == ClaimPermission.Build)
        {
            ...
            if (playerData != null && playerData.inPvpCombat())
            {
    ...
    public void setPermission(@Nullable String playerID, @Nullable ClaimPermission permissionLevel)
    {
        ...
        {
            if (!this.managers.contains(normalized)) this.managers.add(normalized);
        }
        else
    ...
    public boolean contains(Location location, boolean ignoreHeight, boolean excludeSubdivisions) {
        ...
        if (!ignoreHeight) {
            if (this.is3D) {
                ...
                // For non-3D claims, top-level claims span the full height, so only validate world bounds
        ...
        if (excludeSubdivisions && this.children != null && !this.children.isEmpty()) {
            for (Claim child : this.children) {
